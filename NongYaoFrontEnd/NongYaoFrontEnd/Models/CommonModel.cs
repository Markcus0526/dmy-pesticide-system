...
//         }
        ...
        public static string GetStandardStringFromId(long standard_id)
        {
        ...
        public static string GetStandardStringWithoutUnitFromId(long standard_id)
        {
            tbl_standard item = (from m in db.tbl_standards
                                 where m.deleted == 0 && m.id == standard_id
                                 select m).FirstOrDefault();

            return item.quantity + " " + (item.mass == 0 ? "克" : "毫升");
        }

        public static string GetStandardUnitFromId(long standard_id)
        {
            tbl_standard item = (from m in db.tbl_standards
                                 where m.deleted == 0 && m.id == standard_id
                                 select m).FirstOrDefault();

            return UnitModel.GetUnitNameById(item.unit_id);
        }

        public static long GetNongYaoKindFromName(string nongyaoName)
        {
...
//         }
        ...
        public static long GetCurrentUserShopId()
        {
            ...
            return long.Parse(roles[3]);
        }

        public static byte GetCurrentUserLevel()
        {
            string cookieName = FormsAuthentication.FormsCookieName;// FormsAuthentication.FormsCookieName;
            ...
            return byte.Parse(roles[4]);
        }

        public static byte GetCurrentUserRegionCity()
        {
            string cookieName = FormsAuthentication.FormsCookieName;// FormsAuthentication.FormsCookieName;
        ...
        {
            _db = new NongYaoModelDataContext(ConfigurationManager.ConnectionStrings["NongYaoConnectionString"].ConnectionString);
            return _db;
        }
    }
}